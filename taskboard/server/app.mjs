...
function parseTaskFilters(searchParams) {
  const allowed = new Set(["projectId", "status", "threadId", "archived"]);
  for (const key of searchParams.keys()) {
    if (!allowed.has(key)) {
  ...
  const threadIdValue = searchParams.get("threadId");
  ...
  const threadId = threadIdValue === null
    ? undefined
    : stringField(threadIdValue, "threadId", { required: true, maxLength: 256 });
  return { projectId, status: statusValue ?? undefined, threadId, archived };
}
