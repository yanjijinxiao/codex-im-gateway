...
export function userFacingMessageHandlingError(error: unknown): string {
  if (error instanceof CodexThreadStateError) {
    switch (error.code) {
      case "archived":
        return "[codex-channel-bridge] 当前 Codex session 已归档。请先在 Codex App 中取消归档，再重新发送消息。";
      case "missing":
        return "[codex-channel-bridge] 当前 Codex session 已不存在或被删除。请发送 /sessions 重新选择，或发送 /new 新建 session。";
      case "system-error":
        return "[codex-channel-bridge] 当前 Codex session 处于系统错误状态。请先在 Codex App 中打开并处理错误。";
    }
  }
  const message = error instanceof Error ? error.message : String(error);
  if (/DingTalk inbound image download failed/i.test(message)) {
    return "[codex-channel-bridge] 已收到钉钉图片，但下载失败。请重新发送；如果仍失败，请检查机器人是否有消息文件下载权限。";
  }
